...
function validate_user($user, $errors = array()) {
	...
	} elseif (!has_length($user['password'], array('min' => 12)) || !has_length($user['confirm_password'], array('min' => 12))) {
		$errors[] = "Password must be more than 12 characters.";
	}
	if ($user['password'] !== $user['confirm_password']) {
		$errors[] = "Passwords do not match";
	}
	if (preg_match('/[A-Z]/', $user['password']) == 0 ||
		preg_match('/[a-z]/', $user['password']) == 0 ||
		preg_match('/\d/', $user['password']) == 0) {
		$errors[] = "Password must contain at least one of each: uppercase letter, lowercase letter, number.";
	}
	return $errors;
}
